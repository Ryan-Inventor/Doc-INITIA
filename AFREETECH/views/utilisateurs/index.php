    ...
    <div class="flex-1 overflow-x-hidden overflow-y-auto">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                    <h3 class="text-2xl sm:text-3xl font-semibold text-slate-900 tracking-tight">Utilisateurs</h3>
                    <p class="text-sm text-slate-500 mt-1">
                        Gestion des comptes administrateurs et gestionnaires.
                    </p>
                </div>
                <a href="<?php echo url('utilisateurs/create'); ?>" class="inline-flex items-center justify-center rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1">
                    Nouvel utilisateur
                </a>
            </div>
            ...
            <div class="bg-white/80 backdrop-blur rounded-2xl border border-slate-100 shadow-sm">
                <div class="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
                    <h4 class="text-sm font-semibold text-slate-800">Tous les utilisateurs</h4>
                </div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-100 text-sm">
                        <thead class="bg-slate-50">
                            <tr>
                                <th class="px-6 py-3 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wide">Nom</th>
                                <th class="px-6 py-3 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wide">Email</th>
                                <th class="px-6 py-3 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wide">Rôle</th>
                        ...
                        <tbody class="bg-white divide-y divide-slate-50">
                            ...
                            <tr>
                                <td class="px-6 py-4 align-top">
                                    <div class="text-sm font-medium text-slate-900"><?php echo $user['nom_complet']; ?></div>
                                </td>
                                <td class="px-6 py-4 align-top">
                                    <div class="text-sm text-slate-700"><?php echo $user['email']; ?></div>
                                </td>
                                <td class="px-6 py-4 align-top">
                                    <span class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium 
                                        <?php echo $user['role'] === 'admin' ? 'bg-violet-50 text-violet-700' : 'bg-emerald-50 text-emerald-700'; ?>">
                                        <?php echo ucfirst($user['role']); ?>
                                    </span>
                                </td>
                                <td class="px-6 py-4 align-top">
                                    <div class="text-sm text-slate-700"><?php echo ucfirst($user['succursale']); ?></div>
                                </td>
                                <td class="px-6 py-4 align-top text-right whitespace-nowrap">
                                    <a href="<?php echo url('utilisateurs/edit/' . $user['id']); ?>" class="text-xs font-medium text-indigo-600 hover:text-indigo-800 mr-3">Modifier</a>
                                    <?php if($user['id'] != $_SESSION['user_id']): ?>
                                    <a href="<?php echo url('utilisateurs/delete/' . $user['id']); ?>" class="text-xs font-medium text-rose-600 hover:text-rose-800" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet utilisateur ?')">Supprimer</a>
                                    <?php endif; ?>
                                </td>
                            </tr>
                            <?php endforeach; ?>
